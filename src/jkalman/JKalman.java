...
*/      
    ...
    public Matrix Predict(Matrix control) {
        ...
        // P3k: if no correction occured, there's nothing to predict
        if (state_pre == null || state_post == null) return null;
    ...
    public Matrix Correct(final Matrix measurement) {
        ...
        // if no correction occured before, init the Kalman filter
        if (state_pre == null || state_post == null) {
            // following variables must be initialized properly in advance
            state_post = new Matrix(dp, 1); // init by the first measurement!!!

            // hack - initialization of the corrected state (dp > mp ... ?)
            double[][] SP = state_post.getArray();
            double[][] M = measurement.getArray();
            for (int i=0; i < mp; i++) {
                SP[i][0] = M[i][0];
            }

            // init the predicted state by transition_matrix*state_post
            state_pre = transition_matrix.times(state_post);    // new Matrix(dp, 1);
        }
