 ...
 * ------------------------ */
    ...
    /** Element-by-element square root, C = sqrt(A.)
    @return     Sqrt(A.)
    */
    public
    Matrix arraySqrt() {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                if (A[i][j] < 0) A[i][j] = 0; // hm... check it out not to be NaN :(
    ...
    /** Construct an 1-by-n (1 column) matrix no the basis of a comma separated string of doubles. 
    @param s the string (PostgreSQl ARRAY[XXX] or a line of a CSV file).
    */
    public
    Matrix(String s) {
        if (s.charAt(0) == '[' || s.charAt(0) == '{' || s.charAt(0) == '(') s = s.substring(1, s.length()-1);
        String[] sf = s.split("[,;:\\s]+\\s*");
        this.m = 1;
        this.n = sf.length;
        A = new double[1][n];
        for (int i = 0; i < n; i++) {
            A[0][i] = Double.parseDouble(sf[i]);
        }
    }
   ...
   @param n    Number of colums.
    ...
    /** Get a single element from a vector Matrix.
   @param j    Column index.
   @return     A(i,j)
   @exception  ArrayIndexOutOfBoundsException
     */
     ...
     */
    public String toString(int width, int decimals) {
        return toString(width, decimals, ",");
    }
     ...
     * @see print(int w, int d)
     */
    public String toString(int width, int decimals, String separator) {
        DecimalFormat format = new DecimalFormat();
        format.setDecimalFormatSymbols(new DecimalFormatSymbols(Locale.US));
        format.setMinimumIntegerDigits(1);
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                fnum = format.format(A[i][j]); // format the number
                padding = Math.max(1, width - fnum.length()); // At _least_ 1 space
                for (int k = 0; k < padding; k++) {
                     result += " ";
                }
                result += fnum + separator;
            }
            result = result.substring(0, result.length()-separator.length()) + "\n";
        }
        
        return result.substring(0, result.length()-1); // delete the last \n
    }
    ...
    public String toString() {
        ...
        return result.substring(0, result.length()-1);  // delete the last /n
    }
